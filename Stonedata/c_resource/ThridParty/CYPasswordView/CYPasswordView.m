...
- (instancetype)initWithFrame:(CGRect)frame WithMoneyStr:(NSString *)moneyStr
{
...
- (void)setupUI {
    ...
    [self.passwordInputView addSubview:self.bgView];
...
- (void)layoutSubviews {
    ...
    self.imgRotation.centerY = self.passwordInputView.height * 0.3;
...
- (void)showInView:(UIView *)view
{
    ...
    /** 输入框起始frame */
    if (isiPhoneX) {
        self.passwordInputView.height = CYPasswordInputViewHeight + 80;
    }else{
    ...
    maskLayer.path = [UIBezierPath bezierPathWithRoundedRect: self.passwordInputView.bounds byRoundingCorners: UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii: (CGSize){10, 10}].CGPath;
     self.passwordInputView.layer.masksToBounds = YES;
     self.passwordInputView.layer.mask = maskLayer;
    /** 弹出键盘 */
    [self showKeyboard];
}
...
-(void)setMoneyStr:(NSString *)moneyStr{
    _moneyStr = [moneyStr copy];
    self.passwordInputView.moneyStr = moneyStr;
}

-(void)setFeeStr:(NSString *)feeStr{
    _feeStr = [feeStr copy];
    self.passwordInputView.feeStr = feeStr;
}
...
-(UIView *)bgView{
    if (!_bgView) {
        _bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 60, kScreenWidth, 200)];
