//
//  MHHuGuessDetailViewController.m
//  mohu
//
...
@interface MHHuGuessDetailViewController ()<UITableViewDelegate,UITableViewDataSource,MHProductDetailPicViewDelegate>
...
@implementation MHHuGuessDetailViewController
    ...
    if ([self.comefrom isEqualToString:@"list"]) {
        ...
        if ([[NSString stringWithFormat:@"%@",[self.dic valueForKey:@"relation"]] isEqualToString:@"1"]) {
            ...
            if ([GVUserDefaults standardUserDefaults].accessToken) {
                ...
                NSString *str = [self.dic valueForKey:@"status"];if ([str isEqualToString:@"PENDING"]) {
                    [btn setTitle:@"未开始" forState:UIControlStateNormal];
                ...
                [btn setTitle:@"立即发起活动" forState:UIControlStateNormal];
...
-(MHProductShareView *)shareView{
    if (!_shareView) {
        ...
        _shareView = [[MHProductShareView alloc]initWithFrame:CGRectMake(kRealValue(15), kRealValue(28), kScreenWidth - kRealValue(30), kRealValue(500)) dict:self.sharedic.count>0?self.sharedic:self.dic  dic:self.explandDic comefrom:@"hucai" shareId:self.shareID];
//        _shareView.comefrom = @"hucai";
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (self.resttimer == 0) {
            self.hucaiCell.labeltime.text = [NSString stringWithFormat:@"%@ %@",@"距离活动结束还剩",@"00:00:00"];
        }
    ...
    if (indexPath.section == 2) {
        MHProductDetailDesCell *cell =  [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([MHProductDetailDesCell class])];
        cell.selectionStyle= UITableViewCellSelectionStyleNone;
        cell.labeldetail1.text = [self.dic valueForKey:@"productSubtitle"];
        return cell;
    }
    return nil;
