...
//  mohu
//
//  Created by AllenQin on 2018/10/22.
...
@interface MHNewbeeVC ()<UITableViewDataSource,UITableViewDelegate>{
    UILabel            *_titleLabel;
    NSArray            *_titleArr;
    NSArray             *_descArr;
}
...
-(UITableView *)contentTableView{
    if (!_contentTableView) {
        ...
        if ([GVUserDefaults standardUserDefaults].accessToken) {
            if ([[GVUserDefaults standardUserDefaults].userRole isEqualToString:@"1"]) {
                headImageView.image = kGetImage(@"level_header");
                headImageView.frame = CGRectMake(0, 0, kScreenWidth, kRealValue(293));
            }else{
                headImageView.frame = CGRectMake(0, 0, kScreenWidth, kRealValue(250));
                headImageView.image = kGetImage(@"yaoqingyouni");
            }
