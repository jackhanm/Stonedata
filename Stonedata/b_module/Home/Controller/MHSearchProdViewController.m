...
@interface MHSearchProdViewController ()<UITableViewDataSource,UITableViewDelegate,CYLTableViewPlaceHolderDelegate, MHNetworkErrorPlaceHolderDelegate>{
...
@implementation MHSearchProdViewController
...
-(void)requireData{
    [[MHUserService sharedInstance]initwithTypeIdList:@"" name:_descStr brandId:@"" minPrice:@"" maxPrice:@"" order:@"" sort:@"" pageSize:@"10" pageIndex:[NSString stringWithFormat:@"%ld",_index] completionBlock:^(NSDictionary *response, NSError *error) {
        if (ValidResponseDict(response)) {
            if (self.index == 1) {
                [_prodList removeAllObjects];
            }
            
             [_prodList  addObjectsFromArray:[MHProductModel baseModelWithArr:response[@"data"][@"list"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.ProductModel = _prodList[indexPath.row];
    return cell;
}


-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    MHProductModel *model = _prodList[indexPath.row];
