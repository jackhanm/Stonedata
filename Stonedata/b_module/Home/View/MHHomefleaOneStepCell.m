...
@implementation MHHomefleaOneStepCell
...
-(void)createview
{
    ...
    [self.bgView addSubview:self.shopcartCountView];
    
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(kRealValue(16),kRealValue(131) , kRealValue(355-32), 1/kScreenScale)];
    line.backgroundColor = KColorFromRGB(0xe0e0e0);
...
-(void)createviewWithModel:(MHflealistModel *)model
{
    ...
    self.pricelabel.text = [NSString stringWithFormat:@"当前可出售%ld件",model.ownCount];
    NSString *Str2 = [NSString stringWithFormat:@"当前可出售%ld件",model.ownCount];
...
//    _salenumlabel.text = [NSString stringWithFormat:@"回收价 ￥ %@",model.recoverPrice];
    [self.shopcartCountView configureShopcartCountViewWithProductCount:1 productStock:model.ownCount];
    _shopcartCountView.shopcarCountViewEditBlock  = ^(NSInteger count){
        MHLog(@"%ld\n",count);
        if (count > model.ownCount) {
             [self.shopcartCountView configureShopcartCountViewWithProductCount:model.ownCount productStock:model.ownCount];
        }else{
            [self.shopcartCountView configureShopcartCountViewWithProductCount:count productStock:model.ownCount];
...
-(void)layoutSubviews
{
    ...
    [_pricelabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.bgView.left).offset(kRealValue(14));
        make.top.mas_equalTo(self.bgView.mas_top).offset(kRealValue(146));
    }];
    ...
    [self.salenumlabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.mas_equalTo(self.img.mas_bottom).offset(kRealValue(-5));
    }];
    

    [self.shopcartCountView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self.bgView.mas_right).offset(-15);
        make.centerY.equalTo(self.pricelabel.mas_centerY).offset(0);
        make.size.mas_equalTo(CGSizeMake(kRealValue(97), kRealValue(25)));
    }];
...
-(UILabel *)pricelabel
{
    if (!_pricelabel) {
        ...
        _pricelabel.textColor =[UIColor colorWithHexString:@"e51c23"];
