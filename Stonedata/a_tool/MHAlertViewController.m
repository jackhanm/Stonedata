...
+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    ...
    MHAlertViewController *instance = [MHAlertViewController new];
...
//    self.view.backgroundColor = [UIColor clearColor];
    self.view.backgroundColor = [UIColor colorWithHexString:@"000000" andAlpha:0.37];
    //更新标题的frame
...
- (void)creatContentView {
    ...
    _centerImageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"alert_logo"]];
    _centerImageView.frame = CGRectMake(0, -kRealValue(30), kRealValue(80), kRealValue(80));
    _centerImageView.centerX = _contentView.centerX;
    [_shadowView addSubview:_centerImageView];
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        if(i == 0){
            ...
            [btn setTitleColor:[UIColor colorWithHexString:@"#F6AC19"] forState:UIControlStateNormal];
        }
        btn.titleLabel.font = [UIFont fontWithName:kPingFangRegular size:kRealValue(16)];
...
- (void)updateMessageLabelFrame {
    ...
    CGFloat messageY = self.title.length ? CGRectGetMaxY(_titleLabel.frame) : _contentMargin.top;
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [self creatLabelWithFontSize:kFontValue(14)];
...
- (UILabel *)messageLabel {
    if (!_messageLabel) {
        _messageLabel = [self creatLabelWithFontSize:kFontValue(14)];
